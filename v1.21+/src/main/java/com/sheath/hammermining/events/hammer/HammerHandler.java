...
import com.sheath.hammermining.utils.enchantmentUtils.EnchantmentChecker;
...
public class HammerHandler {
    ...
    public static boolean onBlockBreak(World world, PlayerEntity player, BlockPos pos, BlockState state, BlockEntity blockEntity) {
        EFFICIENCY = world.getRegistryManager().get(RegistryKeys.ENCHANTMENT)
                .getEntry(Enchantments.EFFICIENCY).orElse(null);
    ...
    private static void processBlock(ServerWorld world, BlockPos pos, ServerPlayerEntity player, ItemStack tool) {
        ...
        RegistryEntry<Enchantment> SILK_TOUCH = world.getRegistryManager().get(RegistryKeys.ENCHANTMENT)
                .getEntry(Enchantments.SILK_TOUCH).orElse(null);
        RegistryEntry<Enchantment> FORTUNE = world.getRegistryManager().get(RegistryKeys.ENCHANTMENT)
                .getEntry(Enchantments.FORTUNE).orElse(null);
        ...
        tool.damage(1, player,player.getPreferredEquipmentSlot(player.getMainHandStack()));
    }
